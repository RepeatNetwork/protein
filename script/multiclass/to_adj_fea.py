...
position = 1  ### 1,2,3
label_dir = './orig_data/ZF_label.txt'
feature_dir = './orig_data/ZF_feature_merge.txt'
adj_dir = './orig_data/ZF_repeat_distance.txt'
output_label_dir = './label.tsv'
output_feature_dir = './fea.tsv'
output_adj_dir = './adj.tsv'
tale_set = set()
save_fea_list = []
with io.open(label_dir,'r',encoding='utf-8') as fi, io.open(output_label_dir,'w',encoding='utf-8') as fo:
    for i,line1 in enumerate(fi):
        ...
        fo.write(repeat)
        for i in range(4):
            index = 4 *(position-1) + i+1
            fo.write(u'\t'+str(line[index]))
        fo.write(u'\n')
